...
class UnionAuth extends Sso {
    ...
    public function request() {
        ...
        $token = $this->getAccessToken(
            Config::get('auth.stores.wechat.appid'), Config::get('auth.stores.wechat.secret'), Request::get('code')
        );
    ...
    public function getCode(
        string $appid, string $redirect_uri, string $response_type = 'code', string $scope = 'snsapi_base', string $state = ''
    ) {
    ...
    public function getAccessToken(string $appid, string $secret, string $code) {
        $url = 'https://auth.tianfu.ink/auth.php/oauth2/access_token?appid=' . $appid . '&secret=' . $secret . '&code=' . $code . '&grant_type=authorization_code';
    ...
    public function refreshToken(string $appid, string $refresh_token) {
        $url = 'https://auth.tianfu.ink/auth.php/oauth2/refresh_token?appid=' . $appid . '&grant_type=refresh_token&refresh_token=' . $refresh_token;
    ...
    public function getUserInfo(string $access_token, string $openid, string $lang = 'zh_CN') {
        $url = 'https://auth.tianfu.ink/auth.php/oauth2/userinfo?access_token=' . $access_token . '&openid=' . $openid . '&lang=' . $lang;
    ...
    public function verifyToken(string $access_token, string $openid) {
        $url = 'https://auth.tianfu.ink/auth.php/oauth2/verify_token?access_token=' . $access_token . '&openid=' . $openid;
