...
QUERY;
        ...
        if (!empty($res['data']['getAccessTokenByAppSecret'])) {
            ...
            throw new RuntimeException('wrong clientId or secret,please check again!!');
        }
    ...
    private function createOptions($options) {
        if (is_array($options)) {
            if (isset($options['clientId'], $options['secret'])) {
                return $options;
            }
        }
        throw new RuntimeException('Invalid type for client options.');
    }
    ...
    private function checkParams($params) {
        ...
        throw new RuntimeException('wrong type of function args');
    }
...
QUERY;
        ...
        return $this->getHttp($this->_oauthUrl, $body);
...
QUERY;
        ...
        $header = ['Authorization' => "Bearer {$this->_ownerToken}",];

        return $this->getHttp($this->_usersUrl, $body, $header);
    ...
    public function checkLoginStatus() {
        if (empty($this->_userToken)) {
            throw new RuntimeException('user Token is null , please login first');
        }
...
QUERY;
        ...
        $header = ['Authorization' => "Bearer {$this->_userToken}",];
    ...
    public function removeUsers($params) {
        ...
        if (!is_array($params['ids'])) {
            throw new RuntimeException("params['ids'] must be array");
        }
        $query = <<<'QUERY'
...
QUERY;
        ...
        return $this->getHttp($this->_usersUrl, $body);
    }
...
QUERY;
        $body = [
            ...
            'variables' => [
                'email'      => $params['email'],
                'client'     => $this->options['clientId'],
                'verifyCode' => is_string($params['verifyCode']) ? $params['verifyCode'] : (string)($params['verifyCode']),
            ],
        ];
...
QUERY;
    ...
    private function getHttp($url, $data, $header = [], $time = 30000) {
        ...
        $code = curl_errno($ch);
        $error = curl_error($ch);
        throw new RuntimeException("curl internal error, curlErrorCode is {$code},errorMsg: {$error}");
    }
...
QUERY;
        ...
        $body = ['query' => $query, 'variables' => $params];
